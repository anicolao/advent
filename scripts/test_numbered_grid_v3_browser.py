...
with tempfile.TemporaryDirectory(prefix='dot-browser-') as profile:
    ...
    try:
        ...
        assert js('document.querySelectorAll(".cell small").length') == 0
        assert js('numberedGrid.data.instructions[0].name') == 'R1C2'
        ...
        def click_node(node):
            position=js(f"(()=>{{const node={node},r=document.getElementById('canvas').getBoundingClientRect();return {{x:r.left+(100+numberedGrid.data.points[node][0]*36)*r.width/2360,y:r.top+(100+numberedGrid.data.points[node][1]*36)*r.height/2360}}}})()")
        ...
        js('(()=>{const index=numberedGrid.data.instructions.findIndex(i=>{const ops=i.choices[numberedGrid.data.solution[i.cell]-1].ops;return ops.some((op,j)=>op.kind==="join"&&ops[j+1]?.kind==="run")});document.getElementById("key").value=index;document.getElementById("key").dispatchEvent(new Event("change"))})()')
        route=js('numberedGrid.data.real_paths[numberedGrid.state.active]')
        click_node(route[0])
        assert js('numberedGrid.state.segments') == 0
        for node in route[1:]:
            click_node(node)
        ...
        second=js('numberedGrid.data.real_paths[numberedGrid.state.active]')
        click_node(second[0])
        assert js('numberedGrid.state.segments') == len(route)-1
        click_node(second[1])
        ...
        assert js('numberedGrid.state.segments') == 331
        ...
        navigate(OUT/'lookup.html','Boolean(document.querySelector(".lookup"))')
        assert js('document.querySelectorAll(".page").length') == 6
        assert js('Array.from(document.querySelectorAll(".page")).every(p=>p.scrollHeight<=p.clientHeight+1)'), js('Array.from(document.querySelectorAll(".page")).map(p=>[p.scrollHeight,p.clientHeight])')
        assert js('Array.from(document.querySelectorAll(".lookup tbody tr")).length') == 38
        pdf=call('Page.printToPDF',{'preferCSSPageSize':True,'printBackground':True})
        (OUT/'lookup.pdf').write_bytes(base64.b64decode(pdf['data']))
        assert len(re.findall(rb'/Type\s*/Page\b',(OUT/'lookup.pdf').read_bytes())) == 6
        shot=call('Page.captureScreenshot',{'format':'png'})
        (OUT/'printed-lookup-preview.png').write_bytes(base64.b64decode(shot['data']))
        js('document.querySelectorAll(".page")[1].scrollIntoView()')
        shot=call('Page.captureScreenshot',{'format':'png'})
        (OUT/'lookup-table-preview.png').write_bytes(base64.b64decode(shot['data']))
        navigate(OUT/'drawing.html','document.images[0].complete && document.images[0].naturalWidth>0')
        pdf=call('Page.printToPDF',{'preferCSSPageSize':True,'printBackground':True})
        (OUT/'drawing.pdf').write_bytes(base64.b64decode(pdf['data']))
        assert len(re.findall(rb'/Type\s*/Page\b',(OUT/'drawing.pdf').read_bytes())) == 1
        ...
        print('Passed: zero-line initial state; manual numbering and pen lifts; wrong answer draws decoy; animation; reset; all 331 short connections; six-page printed lookup; viewport fit; zoom locator.')
        connection.close()
    finally:
        process.terminate()
