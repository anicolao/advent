...
with tempfile.TemporaryDirectory(prefix='dot-browser-') as profile:
    ...
    try:
        ...
        assert js('document.querySelectorAll(".page").length') == 4
        ...
        assert len(re.findall(rb'/Type\s*/Page\b',(OUT/'lookup.pdf').read_bytes())) == 4
        shot=call('Page.captureScreenshot',{'format':'png'})
        ...
        print('Passed: zero-line initial state; manual numbering and pen lifts; wrong answer draws decoy; animation; reset; all 331 short connections; four-page printed lookup; viewport fit; zoom locator.')
        connection.close()
    finally:
        process.terminate()
