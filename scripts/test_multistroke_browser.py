...
OUT=ROOT/'runs/dot-to-dot-winter-window'
...
with tempfile.TemporaryDirectory(prefix='dot-browser-') as profile:
    ...
    try:
        ...
        assert js('dotPuzzle.state.progress')==277
        assert js('dotPuzzle.state.renderedSegments')==277
        ...
        print('Browser checks passed: 243 points, 277 source-only segments; reveal/reset/scrub; pen-lift pointer drawing with NO connector; wrong-point rejection; undo; animation/pause; hidden source and hints; unchanged dots.')
        connection.close()
    finally:
        process.terminate()
